...
>
	...
	<div class="relative">
		<h2
			class="mx-auto max-w-2xl text-balance bg-gradient-to-b from-violet-50 to-violet-300 bg-clip-text py-2 text-center text-4xl font-medium text-transparent md:text-6xl"
		>
		...
		<div class="mt-20 flex flex-col items-center md:flex-row">
			{#each slice.primary.select as item, index}
				{#if item.icon}
					{#if index === Math.floor(slice.primary.select.length / 2)}
						<StylizedLogoMark />
					...
					{#if index !== slice.primary.select.length - 1}
						<div
							class={clsx(
								'signal-line',
								index >= Math.floor(slice.primary.select.length / 2) ? 'rotate-180' : 'rotate-0'
							)}
						></div>
					{/if}
				{/if}
